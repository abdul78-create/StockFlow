...
export function Suppliers() {
  ...
  const validate = () => {
    ...
    if (!form.companyName.trim() || form.companyName.trim().length < 2) errors.companyName = "Company name must be at least 2 characters";
    ...
    <div className="flex flex-col h-full font-sans selection:bg-gray-900 selection:text-white">
      ...
      <div className="px-6 md:px-10 pt-8 pb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1.5 font-medium">Manage your vendors and supply chain contacts.</p>
        </div>
          ...
          <div className="flex items-center gap-3 p-4 border-b border-gray-100 bg-gray-50/30">
            ...
            <div className="ml-auto flex items-center gap-4">
              <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">
                {loading ? "" : `${filtered.length} suppliers`}
              </span>
              <button 
                ...
                <EmptyState
                  ...
                  description={
                    search
                      ...
                      : "Add your first supplier to start issuing purchase orders."
                  }
                  action={
                    !search ? (
                      ...
                      >
                        <TableCell className="pl-6">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="font-semibold text-gray-900">{s.companyName}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right pr-6">
                          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="sm" onClick={() => openEdit(s)} className="h-8 px-2 text-gray-500 hover:text-gray-900">
      ...
      <Drawer open={drawerOpen} onOpenChange={(open) => { if (!submitting) setDrawerOpen(open); }}>
        <DrawerContent
          ...
          description={
            editingSupplier
              ? "Update vendor details and contact info."
              : "Create a new supplier profile for purchase orders."
          }
        >
          <form onSubmit={handleSubmit} className="space-y-6 pt-6" noValidate>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className={formErrors.companyName ? "border-red-400" : ""}
              />
              {formErrors.companyName && <p className="text-xs font-medium text-red-600">{formErrors.companyName}</p>}
            </div>
            ...
            <div className="space-y-1.5">
              <label className="block text-sm font-semibold text-gray-900">Physical Address</label>
              <Input
                value={form.address}
