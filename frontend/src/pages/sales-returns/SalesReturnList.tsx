...
export function SalesReturnList() {
  ...
  const columns: ColumnDef<SalesReturn>[] = [
    {
      ...
      cell: ({ row }) => (
        <Link to={`/sales-returns/${row.original.id}`} className="font-medium text-blue-600 hover:underline">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem asChild>
                <Link to={`/sales-returns/${row.original.id}`}>
  ...
  const bulkActions = (selectedRows: SalesReturn[]) => (
    <Button 
      variant="outline" 
      size="sm"
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage and track product returns received from customers.</p>
        </div>
        <div className="flex items-center space-x-2">
          <Button onClick={() => navigate('/sales-returns/new')} className="bg-primary hover:bg-primary/90 shadow-sm">
      ...
      >
        {(validData) => (
          <DataTable
            ...
            enableExport={true}
            exportFilename="sales-returns-export.csv"
