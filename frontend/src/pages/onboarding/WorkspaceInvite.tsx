  ...
  const onSubmit = async (data: InviteValues) => {
    try {
      ...
      if (failures.length > 0) {
        ...
        const remainingInvites = failures.map((f) => ({ email: f.email, role: f.role }));
        setValue('invites', remainingInvites);
              ...
              <div key={field.id} className="flex items-start gap-2.5">
                <div className="flex-1 space-y-1">
                  <Input
                    ...
                    className={errors.invites?.[index]?.email ? 'border-destructive' : 'h-11'}
                    disabled={isLoading}
                  />
                ...
                <div className="w-[120px]">
                  <Select
                    onValueChange={(val) => setValue(`invites.${index}.role`, val as any)}
                    value={roleValue}
                    disabled={isLoading}
                  >
