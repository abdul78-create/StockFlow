...
export function PurchaseReturns() {
  ...
  const loadDependencies = async () => {
    try {
      ...
      setPurchaseOrders(poRes.data.data?.orders || (Array.isArray(poRes.data.data) ? poRes.data.data : []));
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!supplierId || !purchaseOrderId || items.some(i => !i.productId || i.quantity <= 0)) {
      toast.error("Please fill all required fields correctly.");
      return;
    }
    try {
      setSubmitting(true);
      await api.post("/purchase-returns", { supplierId, purchaseOrderId, reason, notes, items });
  ...
  const handleAction = async (id: string, action: 'approve' | 'cancel') => {
    try {
      ...
      await api.put(`/purchase-returns/${id}/${action}`);
      toast.success(`Purchase Return ${action}d`);
      fetchReturns();
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col h-full font-sans selection:bg-gray-900 selection:text-white">
      <div className="px-6 md:px-10 pt-8 pb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Purchase Returns</h1>
          <p className="text-sm text-gray-500 mt-1.5 font-medium">Manage returns sent back to suppliers.</p>
        </div>
            ...
            {loading ? (
              ...
              <div className="h-full flex items-center justify-center">
                <EmptyState
                  ...
                  title={search ? "No returns match your search" : "No purchase returns yet"}
                  description={search ? "Try adjusting your search terms." : "Create your first purchase return to a supplier."}
                    ...
                    {paginated.map((r) => (
                      ...
                      >
                        <TableCell className="pl-6">
                          ...
                          <div className="text-xs text-gray-500 mt-0.5">PO: {r.purchaseOrder?.poNumber} · {new Date(r.createdAt).toLocaleDateString()}</div>
                        </TableCell>
                        <TableCell>
                          <div className="font-medium text-gray-700">{r.supplier?.name || 'Unknown'}</div>
                        </TableCell>
      ...
      <Drawer open={createDrawerOpen} onOpenChange={(open) => !submitting && setCreateDrawerOpen(open)}>
        <DrawerContent title="Create Purchase Return" description="Return items back to supplier.">
          <form onSubmit={handleCreateSubmit} className="space-y-6 pt-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-gray-900">Purchase Order <span className="text-red-500">*</span></label>
                <select 
                  required 
                    ...
                    <div className="flex gap-2 items-start">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if(prod) newItems[idx].unitPrice = prod.costPrice || 0;
                          setItems(newItems);
                        }}
            ...
            <div className="space-y-1.5">
              ...
              <Input placeholder="Why are you returning this?" value={reason} onChange={e => setReason(e.target.value)} />
            </div>
